...
import { 
  Search, Mail, Phone, MapPin, BarChart3, TrendingUp, Calendar, 
  Briefcase, Award, Loader2, Eye, RefreshCw, ChevronDown 
} from "lucide-react";
...
export default function RecruiterTeamPage() {
  ...
  useEffect(() => {
    if (user && ["team_lead", "team_manager", "admin"].includes(user.role)) {
      fetchTeam();
    }
  }, [user]);
  ...
  const handleViewPerformance = async (recruiter: any) => {
    ...
    setStats(null);
    try {
      const { data } = await recruitersApi.stats({ user_id: recruiter.id });
      setStats(data);
    } catch (err: any) {
      toast({ 
  ...
  const filtered = teamMembers.filter(r => 
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex flex-col gap-1">
          <h2 className="text-3xl font-bold tracking-tight text-foreground">My Team</h2>
          <p className="text-sm text-muted-foreground font-medium">
            Monitor and analyze your assigned recruitment team's productivity and success.
          </p>
        </div>
        <Button 
          id="refresh-team-btn"
          ...
          onClick={fetchTeam} 
          disabled={loading} 
          className="rounded-xl border-border/50 h-10 px-4"
        >
      ...
      <div className="flex items-center gap-3">
        ...
        <Badge variant="secondary" className="h-11 px-6 rounded-xl bg-primary/5 text-primary border-primary/10 font-semibold" id="team-members-count-badge">
            ...
            columns={[
              ...
              { 
                ...
                render: (r: any) => {
                  ...
                  const social = r.social_profile_url;
          ...
          <DialogHeader className="p-8 pb-4">
            <div className="flex items-center flex-wrap gap-4 mb-2">
              <div className="h-16 w-16 rounded-[1.5rem] bg-primary/10 flex items-center justify-center text-primary text-2xl font-black shadow-inner ring-1 ring-primary/20">
                {selectedRecruiter?.full_name?.[0] || selectedRecruiter?.email?.[0]?.toUpperCase()}
              </div>
              <div className="flex flex-col">
                <DialogTitle className="text-2xl font-black tracking-tighter text-foreground">{selectedRecruiter?.full_name || selectedRecruiter?.email}'s Performance</DialogTitle>
          ...
          <div className="p-8 pt-0 space-y-6">
            {loadingStats ? (
              ...
              <>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="p-5 rounded-3xl bg-muted/40 border border-border/30 flex items-center justify-between group hover:bg-muted/60 transition-colors col-span-2">
