...
# Per-role prefix mapping for display IDs
# User requirements: Candidate: HYRCDTxxxxxx, Recruiter: HYRRECxxxxxx, Interested: HYRLDxxxx
ROLE_PREFIX_MAP = {
    'candidate': 'HYRCDT',
    'recruiter': 'HYRREC',
    'team_lead': 'HYRTLD',
...
class User(AbstractBaseUser, PermissionsMixin):
    ...
    seq_number = models.PositiveIntegerField(null=True, blank=True, editable=False, help_text="Auto-assigned per-role sequential number for display ID")
    ...
    @property
    def display_id(self):
        """Human-readable ID based on user latest requirements."""
    ...
    def save(self, *args, **kwargs):
        if self._state.adding and not self.seq_number and self.role:
            # Assign next sequential number for this role
